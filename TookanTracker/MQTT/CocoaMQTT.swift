...
//    var aliveTimer: MSWeakTimer?
    ...
    public func ping() {
        send(frame: CocoaMQTTFrame(type: CocoaMQTTFrameType.PINGREQ), tag: -0xC0)
    ...
    func sendConnectFrame() {
        ...
        send(frame: frame)
        reader!.start()
        self.delegate?.mqtt(self, didConnect: host, port: Int(port))
    }
    ...
    @nonobjc public func socket(sock: GCDAsyncSocket, didReceiveTrust trust: SecTrust, completionHandler: ((Bool) -> Void)) {
        #if DEBUG
    ...
    public func socket(_ sock: GCDAsyncSocket, didRead data: Data, withTag tag: Int) {
        ...
        case CocoaMQTTReadTag.TAG_PAYLOAD:
            reader!.payloadReady(data: data as NSData)
        }
    }
...
//    }
    ...
    func didReceivePublish(reader: CocoaMQTTReader, message: CocoaMQTTMessage, id: UInt16) {
        ...
        if message.qos == CocoaMQTTQOS.QOS1 {
            _puback(type: CocoaMQTTFrameType.PUBACK, msgid: id)
        } else if message.qos == CocoaMQTTQOS.QOS2 {
            _puback(type: CocoaMQTTFrameType.PUBREC, msgid: id)
        }
    }
    ...
    func _puback(type: CocoaMQTTFrameType, msgid: UInt16) {
        ...
        #endif
        send(frame: CocoaMQTTFramePubAck(type: type, msgid: msgid))
    }
    ...
    func didReceivePubRec(reader: CocoaMQTTReader, msgid: UInt16) {
        ...
        #endif
        _puback(type: CocoaMQTTFrameType.PUBREL, msgid: msgid)
    }
    ...
    func didReceivePubRel(reader: CocoaMQTTReader, msgid: UInt16) {
        ...
        #endif
        _puback(type: CocoaMQTTFrameType.PUBCOMP, msgid: msgid)
    }
